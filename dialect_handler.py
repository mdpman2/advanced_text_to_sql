...
_DIALECT_PATTERNS: Dict[str, re.Pattern] = {
    ...
    # SQL Server (v3.0 최적화: 프리컴파일)
    'limit': re.compile(r'\bLIMIT\s+(\d+)\b', re.IGNORECASE),
}
...
class MySQLDialect(DialectConverter):
    ...
    def convert(self, sql: str, source_dialect: SQLDialect) -> str:
        ...
        if source_dialect == SQLDialect.SQLITE:
            ...
            result = _DIALECT_PATTERNS['strftime'].sub(r"DATE_FORMAT(\2, '\1')", result)
...
class DialectManager:
    ...
    v3.0: MySQL, SQL Server 방언 추가 (총 6종)

    v3.0.0 최적화:
    - get_dialect_hints() if/elif 6단 분기 → _DIALECT_HINTS dict dispatch O(1)
    - 방언별 힐트를 클래스 레벨 dict로 사전 정의
    """
    ...
    def __init__(self):
        self.dialects: Dict[SQLDialect, DialectConverter] = {
            ...
            SQLDialect.MYSQL: MySQLDialect(),          # v3.0 신규
            SQLDialect.SQLSERVER: SQLServerDialect(),  # v3.0 신규
        }
    ...
    def detect_dialect(self, sql: str) -> SQLDialect:
        ...
        # MySQL 특징 (v3.0 신규)
        if "AUTO_INCREMENT" in sql_upper or "IFNULL" in sql_upper or "DATE_FORMAT" in sql_upper:
            return SQLDialect.MYSQL

        # SQL Server 특징 (v3.0 신규)
        if "TOP " in sql_upper or "GETDATE()" in sql_upper or "NVARCHAR" in sql_upper:
            return SQLDialect.SQLSERVER

        # 기본값
        return SQLDialect.SQLITE

    # 방언별 힌트 (if/elif 6단 분기 → dict dispatch 최적화)
    _DIALECT_HINTS: Dict[SQLDialect, List[str]] = {
        SQLDialect.BIGQUERY: [
        ...
        SQLDialect.MYSQL: [
            "MySQL은 백틱(`)으로 식별자를 인용합니다.",
            "문자열 집계: GROUP_CONCAT 함수 (SEPARATOR 지정 가능)",
            "날짜 함수: DATE_FORMAT, DATE_ADD, DATEDIFF",
            "JSON 지원: JSON_EXTRACT, JSON_ARRAYAGG",
            "LIMIT 구문: LIMIT offset, count",
        ],
        SQLDialect.SQLSERVER: [
            "SQL Server는 대괄호([])로 식별자를 인용합니다.",
            "문자열 집계: STRING_AGG 함수 (2017+)",
            "날짜 함수: FORMAT, DATEADD, DATEDIFF, GETDATE()",
            "결과 제한: TOP N 또는 OFFSET-FETCH",
            "문자열 연결: + 연산자 또는 CONCAT()",
        ],
    }
    ...
    def get_dialect_hints(self, dialect: SQLDialect) -> List[str]:
        ...
        (LLM 프롬프트에 포함할 정보)
        """
        return list(self._DIALECT_HINTS.get(dialect, []))
